...
class Bip39VectorsTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $this->testVectors("English", "english", \sakoora0x\BIP39\Language\English::getInstance());
    }
     ...
     * @throws \sakoora0x\BIP39\Exception\Bip39MnemonicException
     */
    private function testVectors(string $language, string $langProp, \sakoora0x\BIP39\Language\AbstractLanguage $wordList): void
    {
        if (!$this->vectors) {
        ...
        for ($i = 0; $i < count($vectors); $i++) {
            ...
            // Entropy to Words
            $secret1 = \sakoora0x\BIP39\BIP39::fromEntropy(hex2bin($vectors[$i][0]), $wordList);
            $this->assertEquals(
            ...
            //  Words to Entropy
            $secret2 = \sakoora0x\BIP39\BIP39::fromMnemonic(explode(" ", $vectors[$i][1]), $wordList, true);
            $this->assertEquals(
                $secret2->entropy,
                $vectors[$i][0],
